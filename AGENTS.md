...
- Electron desktop `node` / `npm` / `npx` PATH bridging — when the packaged `.app` is launched from Finder/Spotlight on macOS, the OS gives it a minimal PATH (`/usr/bin:/bin`). Homebrew, nvm, asdf installs of Node.js are invisible to spawned subprocesses. Two breakages flow from that: (1) backend launcher scripts that do `spawn("node", ...)` can't find Node; (2) most stdio MCP marketplace entries (Slack, GitHub, Figma, etc.) use `command: "npx"`, and when the agent-server tries to spawn them the missing `npx` makes the spawn fail with ENOENT — the SDK reports it as an `error_kind: "connection"` MCP test failure, which the install modal renders as `MCP$TEST_ERROR_CONNECTION` ("Could not reach the server. Check the URL and server type."), a misleading error since no URL is involved. Fix: `electron/main.mjs::ensureNodeAndNpmWrappers()` runs at app startup and probes for `node`/`npm`/`npx` on PATH; for anything missing in a packaged build it writes a thin shell/cmd wrapper to a temp dir and prepends that dir to PATH. The `node` wrapper just runs Electron with `ELECTRON_RUN_AS_NODE=1` (Electron ships its own Node runtime); the `npm`/`npx` wrappers run Electron-as-Node against the bundled npm CLI JS at `process.resourcesPath/npm/bin/{npm,npx}-cli.js`. That bundled npm comes from `scripts/download-npm.mjs` (default pin: `NPM_BUNDLE_VERSION = "10.9.2"`, override with `NPM_VERSION=`) which downloads the npm tarball from registry.npmjs.org into `resources/npm/` (gitignored). `electron-builder.config.mjs` ships `resources/npm/` as an extraResource (~20 MB extracted, all pure JS — its own deps come via npm's `bundledDependencies`). `build:desktop` and `build:desktop:universal` both run `download-npm.mjs` after `download-uv.mjs`. In dev (`npm run desktop`), the probe finds system `node`/`npm`/`npx` and skips wrapper creation entirely. If a fresh packaged build ever ships without the bundled npm dir (e.g. someone forgets to run the download), `ensureNodeAndNpmWrappers` logs a loud `[desktop]` warning instead of writing broken wrappers that would just defer the failure.
