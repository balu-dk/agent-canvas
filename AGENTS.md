...
- **Public-skills cache pre-seeding**: The SDK's `_clone_repository` uses `git clone --depth 1 --branch <ref>`, which fails for raw 40-char commit SHAs (`--branch` only accepts branch/tag names). `preseedExtensionsCache(sha)` in `dev-safe.mjs` works around this by pre-creating the cache at `~/.openhands/cache/skills/public-skills` via `git init` + `git fetch --depth=1 origin <sha>` + `git checkout FETCH_HEAD` before the agent-server starts. Once the cache directory exists, the SDK takes its update path (fetch + checkout) which handles SHAs fine. Called from `dev-safe.mjs` main, `dev-with-automation.mjs` `startAgentServer()`, and `docker/entrypoint.sh`. Without this, on any fresh environment, `EXTENSIONS_REF=<sha>` causes the SDK clone to fail silently and no public skills are loaded — breaking skill-trigger tests like `mock-llm-preset-automation.spec.ts`.
