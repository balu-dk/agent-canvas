...
describe("useLlmConfigured", () => {
  ...
  it("treats a pending ACP agent profile as configured despite no usable LLM", async () => {
    // Agent profiles overlay per conversation without writing the slot, so the
    // applied agent is still OpenHands with no key-less-but-usable LLM here.
    pendingProfile = { engine: "claude-code" };
    mockListProfiles.mockResolvedValue({
      active_profile: "gpt-5.5",
      profiles: [
        {
    ...
    // The ACP profile owns its own credential — no OpenHands LLM required.
    expect(result.current.isConfigured).toBe(true);
  });

  it("does not treat a pending OpenHands agent profile as self-configured", async () => {
