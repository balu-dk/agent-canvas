...
3. **Run the server**: clone this repo and run `npm run dev -- --sandbox none --static`
...
`npm run dev -- --sandbox none --static` spins up the static frontend, the agent
...
> [!WARNING]
> `npm run dev -- --sandbox none --static` runs the agent server **directly on the host**.
